...
class HDFE(nn.Module):
    def __init__(self, alpha, dim):
        ...
        self.dim = dim
        self.Theta = torch.randn(3, dim) * alpha
        self.Theta = nn.Parameter(self.Theta, requires_grad=False)

        self.fc1 = ComplexLinear(dim, 128)
        self.relu = ComplexReLU()
        
        self.fc_real = nn.Linear(128, 128)
        self.fc_imag = nn.Linear(128, 128)
    
    def forward(self, x):
        with torch.no_grad():
            x = torch.exp(1j * (x @ self.Theta * self.alpha)) 
            x = torch.sum(x, dim=-2)
        ...
        x = self.fc_real(x.real) + self.fc_imag(x.imag)
        return x
...
class Network(nn.Module):
    def __init__(self, num_in=1, num_knn=16, decode_knn=16, use_HDFE=False):
    ...
    def forward(self, pos, mode_test=False):
        ...
        if self.use_HDFE:
            HDFE_feat = self.HDFEEncoder(nn_pc)                                     # (B, n, K, 3) -> (B, n, C)
            feat = feat + HDFE_feat
